  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<LinksOverviewBloc, LinksOverviewState>(
      listener: (context, state) {
        ...
        if (state.status == LinksOverviewStatus.deleteSuccess) {
          context.read<LinksOverviewBloc>().add(
                LinksOverviewSubscriptionRequested(),
              );
        }
        if (state.status == LinksOverviewStatus.deleteFailure) {
          Navigator.pop(context);
          SnackBar snackbar = createSnackbar(
            context,
            l10n.deleteLinkErrorSnackbarText,
            true,
          );
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () => {
                  context.read<LinksOverviewBloc>().add(
                        LinksOverviewLinkDeleted(widget.link),
                      ),
                },
              ),
              Divider(),
